...
def get(args: argparse.Namespace) -> None:
    ...
    if args.talos is not None:
        write_talos(args.talos, correlations, selector.atoms, args.entry)
...
def write_talos(outfile: TextIO, correlations: Correlations, atoms: Tuple[str], entry_id: str) -> None:
    '''Helper function for outputting TALOS files when --talos is passed.
    '''
    name, sequence = bmrb.get_sequence(entry_id)
    three_to_one = dict(zip(constants.ONE_LETTER_TO_THREE_LETTER.values(), constants.ONE_LETTER_TO_THREE_LETTER.keys()))

    header = f"""REMARK Generated by nightshift
REMARK Data from BMRB entry id {entry_id}: {name}
...
"""
    with outfile as talos_file:
        talos_file.write(header)
        for sequence_number, residue_type, chemical_shifts in correlations:
            for atom, shift in zip(atoms, chemical_shifts):
                atom = 'HN' if atom == 'H' else atom
                atom = 'HA' if atom == 'HMETHYL' else atom
                atom = 'CA' if atom == 'CMETHYL' else atom
                talos_file.write(f'{sequence_number} {three_to_one[residue_type]} {atom} {shift:8.3f}\n')
